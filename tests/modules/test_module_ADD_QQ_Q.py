...
def test_add_qq_q_both_positive():
...
def test_add_qq_q_both_negative():
    a = Rational.from_str("-1/2")
    b = Rational.from_str("-1/3")
    result = ADD_QQ_Q(a, b)
    assert str(result) == "-5/6"
...
def test_add_qq_q_positive_and_negative_positive_bigger():
...
def test_add_qq_q_positive_and_negative_negative_bigger():
...
@pytest.mark.parametrize("iteration", range(10))
def test_add_qq_q_random_with_negative(iteration):
...
def test_add_qq_q_huge_numbers():
    base_num = "9" * 50
    base_den = "7" * 50
    a = Rational.from_str(f"{base_num}/{base_den}")
    b = Rational.from_str(f"{base_num}/{base_den}")
    expected = Fraction(int(base_num), int(base_den)) * 2
    result = ADD_QQ_Q(a, b)
    assert str(result) == f"{expected.numerator}/{expected.denominator}"


def test_add_qq_q_huge_numbers_opposite_signs():
    big_num = "9" * 60
    small_num = "1" + "0" * 59
    common_den = "3" * 60
...
def test_add_qq_q_invalid_input():
    with pytest.raises(ValueError):
        ADD_QQ_Q("not rational", Rational.from_str("1/2"))  # type: ignore

    with pytest.raises(ValueError):
        ADD_QQ_Q(Rational.from_str("1/2"), "not rational")  # type: ignore

    with pytest.raises(ValueError):
        ADD_QQ_Q(None, Rational.from_str("1/2"))  # type: ignore


def test_add_qq_q_commutative():
    a = Rational.from_str("13/37")
    b = Rational.from_str("42/67")
    result1 = ADD_QQ_Q(a, b)
    result2 = ADD_QQ_Q(b, a)
    assert str(result1) == str(result2)


def test_add_qq_q_associative():
    a = Rational.from_str("5/2")
    b = Rational.from_str("11/61")
    c = Rational.from_str("4/20")

    ab = ADD_QQ_Q(a, b)
    result1 = ADD_QQ_Q(ab, c)

    bc = ADD_QQ_Q(b, c)
    result2 = ADD_QQ_Q(a, bc)

    assert str(result1) == str(result2)


def test_add_qq_q_edge_cases():
    max_r = Rational.from_str(f"{'9' * 50}/1")
    min_r = Rational.from_str(f"-{'9' * 50}/1")
    result = ADD_QQ_Q(max_r, min_r)
    assert str(result) == "0"

    assert str(ADD_QQ_Q(Rational.from_str("0/1"), Rational.from_str("1/1"))) == "1"

    assert str(ADD_QQ_Q(Rational.from_str("-1/2"), Rational.from_str("1/2"))) == "0"
