...
def test_deg_p_n_constant():
    p = Polynom([Rational.from_str("5")])
    result = DEG_P_N(p)
    assert str(result) == "0"


def test_deg_p_n_linear():
    p = Polynom([Rational.from_str("2"), Rational.from_str("3")])
    result = DEG_P_N(p)
    assert str(result) == "1"
...
def test_deg_p_n_high_degree():
    coeffs = [Rational.from_str(str(i)) for i in range(10)]
    p = Polynom(coeffs)
    result = DEG_P_N(p)
    assert str(result) == "9"
...
def test_deg_p_n_invalid_input():
    ...
    with pytest.raises(ValueError):
        DEG_P_N(42)  # type: ignore

    with pytest.raises(ValueError):
        DEG_P_N(None)  # type: ignore


def test_deg_p_n_leading_zeros_normalized():
    p = Polynom(
        [
            Rational.from_str("1"),
            Rational.from_str("2"),
