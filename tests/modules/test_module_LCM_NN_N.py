...
def test_LCM_NN_N():
    ...
    assert str(LCM_NN_N(n1, n2)) == "100"
    assert str(LCM_NN_N(n1, n3)) == "100"


@pytest.mark.parametrize("iteration", range(5))
def test_LCM_NN_N_random(iteration):
    ...
    assert str(LCM_NN_N(n1, n2)) == str(n3)


def test_LCM_NN_N_huge_numbers():
    n1 = Natural.from_str("1" + "0" * 1000000)
    n2 = Natural.from_str("1" + "0" * 100000)
    assert str(LCM_NN_N(n1, n2)) == str(n1)


def test_LCM_NN_N_Fibonacci():
    n1 = Natural.from_str("354224848179261915075")
    n2 = Natural.from_str("21892299583455516902")
    n3 = 354224848179261915075 * 21892299583455516902
    assert str(LCM_NN_N(n1, n2)) == str(n3)


def test_LCM_NN_N_equal_numbers():
    n1 = Natural.from_str("12345")
    n2 = Natural.from_str("12345")
    assert str(LCM_NN_N(n1, n2)) == "12345"


def test_LCM_NN_N_zero_cases():
    n_val = Natural.from_int(42)
    n_zero = Natural.from_int(0)
    with pytest.raises(ValueError):
        LCM_NN_N(n_val, n_zero)
    with pytest.raises(ValueError):
        LCM_NN_N(n_zero, n_zero)


def test_LCM_NN_N_two_zero():
